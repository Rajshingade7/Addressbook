...
var AddressBook = /** @class */ (function () {
    ...
    AddressBook.prototype.readFromJson = function (filePath) {
        var _this = this;
        fs.readFile(filePath, 'utf8', function (err, data) {
            if (err) {
            ...
            try {
                ...
                jsonData.forEach(function (contactData) {
                    var firstName = contactData.firstName, lastName = contactData.lastName, address = contactData.address, city = contactData.city, state = contactData.state, zip = contactData.zip, phone = contactData.phone, email = contactData.email;
                    var contact = new Contact(firstName, lastName, address, city, state, zip, phone, email);
                    _this.addContact(contact);
                });
    ...
    AddressBook.prototype.writeToJson = function (filePath) {
        var jsonData = [];
        for (var addressBookName in addressBookSystem.addressBooks) {
            if (Object.prototype.hasOwnProperty.call(addressBookSystem.addressBooks, addressBookName)) {
                var addressBook = addressBookSystem.addressBooks[addressBookName];
